...
class CameraActivity : AppCompatActivity() {
    ...
    private val cameraStateCallback: CameraDevice.StateCallback =
        object : CameraDevice.StateCallback() {
            ...
            override fun onError(camera: CameraDevice, error: Int) {
                ...
                Toast.makeText(
                    this@CameraActivity,
                    getString(R.string.open_camera_error_tip),
                    Toast.LENGTH_SHORT
                ).show()
        ...
        when (state.get()) {
            ...
            STATE_PICTURE_TAKEN -> {
                ...
                if (binding.mTextureView.isAvailable) {
                    ...
                    binding.mTextureView.surfaceTextureListener =
                        this@CameraActivity.surfaceTextureListener
                }
            }
        }
    }
        ...
        if (requestCode == CameraConfig.CAMERA_PERMISSION_CODE) {
            if (grantResults.size >= 1) {
                ...
                if (cameraGranted && sdGranted && audioGranted) {
                    ...
                    //没有权限
                    Toast.makeText(this, getString(R.string.no_permission_tip), Toast.LENGTH_SHORT)
                        .show()
                    finish()
                }
            }
    ...
    private fun init() {
        binding.mTvRecordTip.text =
            "${if (CameraConfig.IS_ALLOW_PHOTO) getString(R.string.photo_tip) else ""}${if (CameraConfig.IS_ALLOW_PHOTO && CameraConfig.IS_ALLOW_RECORD) "," else ""}${if (CameraConfig.IS_ALLOW_RECORD) getString(
                R.string.record_tip
            ) else ""}"
        initCamera()
        initTouchListener()
    ...
    private fun initCamera() {
        ...
        //初始化传感器定位
        sensorOrientation = CameraConfig.getCurrentCameraCameraCharacteristics()
    ...
    private fun openCamera() {
        ...
        if (!PermissionUtil.checkPermission(this)) {
            ActivityCompat.requestPermissions(
                ...
                CameraConfig.CAMERA_PERMISSION_CODE
            )
        } else {
            try {
                // 3秒内打开摄像头
    ...
    private fun initPreview() {
        ...
        if (!::previewSize.isInitialized) {
            var size = CameraUtil.getMaxOptimalSize(
                CameraConfig.getCurrentCameraCameraCharacteristics(), SurfaceTexture::class.java,
        ...
        if (!::recordSize.isInitialized) {
            val mRecordSize = CameraUtil.getMinOptimalSize(
                CameraConfig.getCurrentCameraCameraCharacteristics(), SurfaceTexture::class.java,
                binding.mTextureView.height, binding.mTextureView.width
            )
            if (mRecordSize == null) {
        ...
        when (state.get()) {
            STATE_PICTURE_TAKEN -> {
                ...
                captureImage?.let { mImage ->
                    ...
                    val filePath =
                        "${CameraConfig.getSaveDir(this@CameraActivity)}/img_$timeStamp.jpg"
            ...
            STATE_RECORD_TAKEN -> {
                ...
                intent.putExtra(
                    JCamera.CAPTURE_RESULT,
                    if (!comRecordPath.isBlank()) comRecordPath else recordPath
                )
                setResult(Activity.RESULT_OK, intent)
                finish()
            }
...
//                    }
        ...
        if (state.get() == STATE_RECORDING) {
            ...
            try {
                ...
                mp4Composer = Mp4Composer(recordPath, comRecordPath)
                    .rotation(correctRecord()).flipHorizontal(!isBackCamera)
                        ...
                        override fun onCompleted() {
                            ...
                            if (Build.VERSION.SDK_INT >= 24) {
                                uri = FileProvider.getUriForFile(
                                    this@CameraActivity,
                                    CameraProvider.getFileProviderName(this@CameraActivity),
                                    File(comRecordPath)
                                )
                            } else {
                ...
                Toast.makeText(
                    this@CameraActivity,
                    getString(R.string.record_time_short),
                    Toast.LENGTH_SHORT
                ).show()
                File(recordPath).delete()
                previewSession.apply {
    ...
    private fun correctRecord(): Rotation {
        ...
        when (CameraUtil.getJpegOrientation(
            CameraConfig.getCurrentCameraCameraCharacteristics(),
            recordOrientation
        )) {
